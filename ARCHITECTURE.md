...
The prepared [Sprint 7 Engineering Specification](SPRINT7_SPECIFICATION.md) proposes an optional provider-neutral assistance layer; no Sprint 7 runtime is implemented and current metadata remains `0.7.0-alpha.1`. The future dependency direction is `UI -> assistance operators -> assistance coordinator -> pure context/validation/provider interfaces -> existing Sprint 5/6 coordinators`. Provider adapters are Blender-reference-free and cannot call operators. Context contains only bounded consented JSON-safe summaries/evidence identities, never raw geometry. Provider output is untrusted data and becomes previewable only after strict decoding, schema/semantic/evidence validation, and exact resolution to a current existing candidate or strategy.
