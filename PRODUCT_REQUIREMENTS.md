...
Sprint 7 runtime implementation has not started, and current metadata remains `0.7.0-alpha.1`. Provider/model, backend/direct/local, BYOK/hosted, retention, production cost/quota, and initial execution scope remain explicit decision gates with live adapters disabled by default. Sprint 8 reference/vision work, Sprint 9 procedural creation, Sprint 10 commercial systems, raw geometry upload, hidden network, arbitrary code, automatic execution/source replacement, slicing/G-code/printer control, physical validation, cultural or geometry correctness, global optimality, and print-success guarantees are not part of this specification milestone.
