...
| ID | Requirement | User value | Status | Target milestone | Acceptance evidence |
|---|---|---|---|---|---|
| FR-AI-001 | Convert natural-language intent into a versioned structured-command schema, not executable code. | Makes recommendations machine-checkable. | Implemented | Sprint 7 | Schema validation, malformed-output, and injection tests |
| FR-AI-002 | Extract only bounded, consented scene and diagnostic context required for a request. | Improves relevance while limiting data exposure. | Implemented | Sprint 7 | Context allow-list and privacy tests |
| FR-AI-003 | Generate recommendations with reasons, assumptions, confidence, and unmet prerequisites. | Lets users judge uncertainty. | Implemented | Sprint 7 | Evaluation set and human review rubric |
| FR-AI-004 | Permit only allow-listed deterministic operations with validated parameters. | Prevents uncontrolled scene or system actions. | Implemented | Sprint 7 | Deny-by-default security suite |
| FR-AI-005 | Show a preview and require confirmation before execution. | Keeps the artist in control. | Implemented | Sprint 7 | UI state, rejection, and cancellation tests |
| FR-AI-006 | Record prompts, structured responses, approvals, operations, and outcomes according to privacy policy. | Supports audit and troubleshooting. | Implemented | Sprint 7 | Audit-schema and redaction tests |
| FR-AI-007 | Abstract model providers behind a stable internal interface. | Avoids product lock-in and keeps policy consistent. | Implemented | Sprint 7 | Provider contract and failure-mode tests |
| FR-AI-008 | Evaluate bring-your-own API credentials and a hosted option without committing to either prematurely. | Supports different privacy and commercial needs. | BYOK selected; hosted deferred | Sprint 7 decision gate | Security, cost, support, and consent review |
...
| FR-AI-010 | Never execute arbitrary Python, downloaded code, or a model-authored command outside the allow-list. | Establishes the core AI security boundary. | Implemented | Sprint 7 and all AI releases | Static audit, adversarial tests, and release security gate |
...
# Sprint 7 Specification Status

The implementation-ready [Sprint 7 Engineering Specification](SPRINT7_SPECIFICATION.md) now defines **AI Recommendation Foundation** from Journey D and `FR-AI-001` through `FR-AI-010`. The supported core is optional provider-neutral structured recommendations over bounded consented current evidence; recommendations resolve exact existing Sprint 5 candidates or Sprint 6 strategies, remain untrusted until strict local validation, require preview and fresh approval, and delegate any mutation to the existing protected workspace/checkpoint path.

Sprint 7 is implemented in the current `0.8.0-alpha.1` worktree as optional OpenAI-first, provider-neutral recommendation assistance. The approved alpha deployment is direct explicit BYOK HTTPS with environment or session-only credentials, no backend/account/billing/telemetry, one stateless request, zero automatic retries, strict local JSON/evidence/target validation, preview plus fresh approval, existing Sprint 5/6 execution delegation, redacted export, and deterministic offline fallback. Sprint 8 reference/vision work, Sprint 9 procedural creation, Sprint 10 commercial systems, raw geometry upload, hidden network, arbitrary code, automatic execution/source replacement, slicing/G-code/printer control, physical validation, cultural or geometry correctness, global optimality, and print-success guarantees remain out of scope.
